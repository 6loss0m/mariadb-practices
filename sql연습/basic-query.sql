...
-- insert : DML(C)
insert into pet values('예원이', '장은영', 'people', 'F', '2000-11-03', null);

-- select : DML(R)
select * from pet;

-- update : DML(U)
update pet set name='애옹이' where owner='장은영';
select * from pet; 

-- delete : DML(D)
delete from pet where name='애옹이';
...
-- 문3) 종이 뱀이거나 새인 애들은?
select * from pet where species = 'sname' or species = 'bird';
...
-- select절에 집계함수가 있으면 어떤 칼럼도 올 수 없다.
